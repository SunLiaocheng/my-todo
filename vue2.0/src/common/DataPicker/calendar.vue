<template>
    <div @click.stop=""  @touchstart.stop="" class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
        ...
        <tr v-for="(k1,day) in days">
            ...
            @click="select(k1,k2,$event)" @touchstart="select(k1,k2,$event)"><span>
            {{child.day}}</span>
        ...
        <div class="calendar-time" v-show="type=='datetime' && twoTime">
            <div class="timer">
                <input type="number" v-model="hour2" value="{{hour2}}" min="0" max="23" maxlength="2" number>
                时
                <input type="number" v-model="minute2" value="{{minute2}}" min="0" max="59" maxlength="2" number>
                分
                <input type="number" v-model="second2" value="{{second2}}" min="0" max="59" maxlength="2" number>
                秒
            </div>
        </div>
...
module.exports = {
    ...
    created:function(){
        ...
        if(that.value!=""){
            ...
            that.hour2 = that.zero(now.getHours());
            that.minute2 = that.zero(now.getMinutes());
            that.second2 = that.zero(now.getSeconds());
            console.log(that.year,that.month,that.day)
            if(that.range){
                // 这是保证未选择时间点击确认时的显示问题
                that.rangeBegin=[that.year,that.month+1,that.day,that.hour,that.minute,that.second];
                that.rangeEnd=[that.year,that.month+1,that.day,that.hour,that.minute,that.second];
    ...
    methods:{
        ...
        render:function(y,m){
            ...
            //当前日期后禁止选择
            if(that.disabledAfter){
                var len1=temp.length,
                    len2=temp[0].length;
                var yearNow=new Date().getFullYear();
                var monthNow=new Date().getMonth();
                var dayNow=new Date().getDate()-1;//-1 包括当前日期
                if(that.year>yearNow){
                    for(var m=0;m<len1;m++){
                        for(var n=0;n<len2;n++){
                            temp[m][n].disabled=true
                        }
                    }
                }else if(that.year==yearNow){
                    if(that.month>monthNow){
                        for(var m=0;m<len1;m++){
                            for(var n=0;n<len2;n++){
                                temp[m][n].disabled=true
                            }
                        }
                    }else if(that.month==monthNow){
                        for(var m=0;m<len1;m++){
                            for(var n=0;n<len2;n++){
                                if(temp[m][n].day>dayNow){
                                    temp[m][n].disabled=true
                                }
                            }
                        }
        ...
        ok:function(){
            var that=this,_statrt='',_end='';

            if(that.range){
                that.value=that.output(that.rangeBegin)+" ~ "+that.output(that.rangeEnd);
                if(that.output(that.rangeBegin).indexOf('undefined')>-1) {
                    that.value = '';
                }
            }else{
            ...
            that.show=false;
            if(that.output(that.rangeBegin).indexOf('undefined') == -1) {
                _statrt = that.output(that.rangeBegin).replace('/','-').replace('/','-');
                _end = that.output(that.rangeEnd).replace('/','-').replace('/','-');
            } else {
                _statrt = '';
                _end = '';
            }
            this.begin = that.output(that.rangeBegin)
            this.end = that.output(that.rangeEnd)
            if(that.type=="datetime" && that.range && that.twoTime) {
                var beginT = that.output(that.rangeBegin).split(' ')[0],
                    endT = that.output(that.rangeEnd).split(' ')[0]
                that.value = beginT+' '+that.outTime([that.hour,that.minute,that.second])+' ~ '+endT+' '+that.outTime([that.hour2,that.minute2,that.second2])
            }
        },
        cancel:function(){
            this.show=false;
        ...
        outTime: function (args) {
            args[0] = args[0]>9?args[0]:'0'+args[0]
            args[1] = args[1]>9?args[1]:'0'+args[1]
            args[2] = args[2]>9?args[2]:'0'+args[2]
